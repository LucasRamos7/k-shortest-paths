...
//void YenTopKShortestPathsAlg::run(BaseVertex* pSource, BaseVertex* pTarget, double Kf, int* nPaths) {
void YenTopKShortestPathsAlg::run(BaseVertex* pSource, BaseVertex* pTarget, int Kf, int* nPaths) {
	m_pSourceVertex = pSource;
	m_pTargetVertex = pTarget;
	//Possible memory leak
	
	m_pGraph = new Graph(cGraph);
	
	_init();

	double shortest_path_weight = get_shortest_path(pSource, pTarget)->Weight();
	//double max_path_weight = Kf * shortest_path_weight;
	double cur_path_weight = 0;
	int larger_path_length = 0;
	
	BasePath* current_Path;

	//std::cout << "\nTolerancia: " << (Kf - 1) * 100 << "%";
	std::cout << "\n\nCusto do caminho mais curto: " << shortest_path_weight;
	//std::cout << "\n\nCusto maximo permitido: " << max_path_weight;
	std::cout << "\n\n";

	int cont = 0;

	while (has_next() && /*cur_path_weight <= max_path_weight &&*/ cont < Kf) {

		current_Path = next();						// Determina qual é o próximo caminho mais curto
		cur_path_weight = current_Path->Weight();	// Pega o custo deste caminho


		//if (cur_path_weight <= max_path_weight){					// Se o custo do caminho atual for menor que o máximo:
		if (cont < Kf) {											// Se o custo do caminho atual for menor que o máximo:

			//current_Path->PrintOut(cout);							// Imprime o caminho no terminal
			WritePaths(current_Path, cont);							// Escreve os caminhos no arquivo de saida
			cont++;
			...
			if (current_Path->length() > larger_path_length) {
				larger_path_length = current_Path->length();		// Atualiza o comprimento do maior caminho
			}
		}
	...
	*nPaths = *nPaths + cont;
	...
	FinishOutput(cont, larger_path_length);		// Cria uma matriz com os caminhos para padronizar o arquivo de saída
